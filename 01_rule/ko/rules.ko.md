...
이 문서는 `01_rule/ai/rules.md`의 사람용 한국어 검토본입니다. AI가 실제 작업 기준으로 삼는 원문은 `01_rule/ai/rules.md`입니다.
...
01_rule/ai/instructions/*.md <-> 01_rule/ko/팀원/*.ko.md
...
AI 토큰 절약 또는 AI 작업 라우팅을 주목적으로 만든 Markdown 문서는 영어를 기본으로 작성합니다.

`01_rule/` 밖의 AI 보조 문서는 별도 한국어 대응본을 의무로 만들지 않습니다.

`01_rule/`은 예외입니다. `01_rule/` 아래의 규칙 관련 문서는 기존처럼 영어/한국어 검토본 동기화 규칙을 따릅니다.

## 4. Ryze 인코딩 검토

## 추가 규칙: 삭제대기 보관

이 저장소에서 사용자가 "삭제"라고 말하거나 Agent에게 파일 삭제를 요청하면, Agent는 이를 영구 삭제가 아니라 `99_아카이브/삭제대기/`로 이동하라는 요청으로 해석합니다. AI Agent는 파일을 직접 영구 삭제하지 않습니다.
...
파일명이 `-이력.md` 또는 `-이력.ko.md`로 끝나는 이력 Markdown 문서는 일자별 섹션으로 관리합니다. 각 일자 섹션은 동일한 열을 가진 별도 표를 사용하며, 최신 일자 섹션이 위로 오도록 정렬합니다.
...
```text
{한국어파일명}.md = 한국어 원본문서
{english-name}.md = 영어 또는 언어 중립 원본문서
{english-name}.ko.md = 영어 원본문서의 한국어 번역본
```

파일명이 이미 한국어라면 `.ko`를 붙이지 않습니다.

`.ko.md`는 영어 원본문서의 한국어 번역본 또는 검토본일 때만 사용합니다.

루트 `README.md`는 명시적 예외이며, 저장소의 기본 사람용 소개 문서로 한국어로 작성할 수 있습니다.

AI-facing 룰 문서는 `.ko.md`로 끝나는 한국어 번역본 또는 검토본이 아닌 이상 영어로 유지합니다.

### 파일 성격별 명명 규칙

파일 목적에 따라 아래 명명 규칙을 사용합니다.

| 파일 성격 | 명명 규칙 | 언어 기준 | 한국어 대응본 |
| --- | --- | --- | --- |
| AI 라우팅 인덱스 | `INDEX.md` | 영어 기본 | `01_rule/` 밖에서는 의무 아님 |
| 한국어 원본 사람용 문서 | `{한국어파일명}.md` | 한국어 | `{한국어파일명}.ko.md`를 만들지 않음 |
| 영어 또는 언어 중립 원본문서 | `{english-name}.md` | 영어 또는 언어 중립 | 한국어 번역본/검토본이 필요할 때만 `{english-name}.ko.md` 생성 |
| 영어 원문의 한국어 번역본 | `{english-name}.ko.md` | 한국어 | `{english-name}.md`와 대응해야 함 |
| `01_rule/ai/` 아래 규칙 문서 | `{english-name}.md` | 영어 | 동기화 쌍이 있으면 `01_rule/ko/` 아래 한국어 검토본 필수 |
| `01_rule/agents/` 아래 Agent 역할 카드 | `{english-name}.md` | 영어 | `01_rule/ko/agents/**/*.ko.md` 필수 |
| 회의록 | `{주제}-회의록-{YYYY-MM-DD}.md` | 한국어 기본 | 녹취록은 별도 파일이 아니라 같은 회의록 안에 포함 |
| 이력 문서 | `{주제}-이력.md` 또는 `{topic}-이력.ko.md` | 문서 원천에 맞춤 | 날짜별 섹션, 최신 날짜 우선 |
| 삭제대기 아카이브 묶음 | `{대상}-{사유}-{YYYY-MM-DD}/` | 식별 가능한 경로 | `90_기록실/삭제대기/삭제대기-이력.ko.md`에 기록 |

규칙이 충돌하면 더 구체적인 파일 성격별 규칙을 우선합니다.
...
이 파일명 규칙은 한국어 회의록 운영 문서의 기본 규칙입니다.
